...
module.exports = function (router) {
	...
	router.post('/application/v0/select-route-a', function (req, res) {
		...
		if (selected_route === 'main') {
			...
			res.redirect('/application/v0/org-ico')
			...
			res.redirect('/application/v0/select-route-error')
		}
		
	})
  
	// ICO number?
	router.post('/application/v0/org-ico-a', function (req, res) {
		req.session.data['tl_org_details'] = 'inprogress'

		let org_ico = req.session.data['org-ico']

		if (org_ico === '12345678') {
			res.redirect('/application/v0/org-ukprn')
		} else {
			res.redirect('/application/v0/org-error-ico')
		}
	})

	// UKPRN?
	router.post('/application/v0/org-ukprn-a', function (req, res) {

		let org_ukprn = req.session.data['org-ukprn']

		if (org_ukprn === '11110000') { // Company - Happy Path
			res.redirect('/application/v0/org-confirmorgdetails')
		} else if (org_ukprn === '11110001') { // Company - Active with no website
			res.redirect('/application/v0/org-confirmorgdetails')
		} else if (org_ukprn === '11110002') { // Company - Inactive
			res.redirect('/application/v0/org-shutter-inactivecompany')
		} else if (org_ukprn === '11110003') { // Company - Also a charity
			res.redirect('/application/v0/org-confirmorgdetails')
		} else if (org_ukprn === '11110004') { // Not a company
			res.redirect('/application/v0/org-legalstatus')
		} else {
			res.redirect('/application/v0/org-error-ukprn')
		}
	})

	// Confirm company details
	router.post('/application/v0/org-confirmorgdetails-a', function (req, res) {

		res.redirect('/application/v0/org-trading')

	})

	// Started trading date
	router.post('/application/v0/org-trading-a', function (req, res) {
		...
		if (req.session.data['org-trading-day'] == "" || req.session.data['org-trading-month'] == "" || req.session.data['org-trading-year'] == "") {
			res.redirect('/application/v0/org-error-trading')
		} else {
			req.session.data['tl_org_details'] = 'completed'
			req.session.data['tl_org_people'] = 'next'
			res.redirect('/application/v0/org-peopleincontrol')
		}
	})
	
	// Confirm people in control
	router.post('/application/v0/org-peopleincontrol-a', function (req, res) {
		req.session.data['tl_org_people'] = 'completed'
		req.session.data['tl_org_classification'] = 'next'
		res.redirect('/application/v0/org-classification')
	})


	// Organisation classification / type
	router.post('/application/v0/org-classification-a', function (req, res) {
		...
		if (org_orgtype === 'education') {
			...
			res.redirect('/application/v0/org-classification-education')
			...
			res.redirect('/application/v0/org-classification-psb')
		} else if (org_orgtype === 'training') {
			req.session.data['tl_org_classification'] = 'inprogress'
			req.session.data['fha-exempt'] = 'no'
			res.redirect('/application/v0/org-classification-training')
		} else {
			res.redirect('/application/v0/org-error-classification')
		}
	})


	// Organisation type = Education
	router.post('/application/v0/org-classification-education-a', function (req, res) {
		...
		res.redirect('/application/v0/org-fundedby')

	})

	// Organisation type = Public Sector Body
	router.post('/application/v0/org-classification-psb-a', function (req, res) {
	...
	router.post('/application/v0/org-classification-training-a', function (req, res) {
	...
	// Funded by
	router.post('/application/v0/org-fundedby-a', function (req, res) {
		...
		if (org_fundedby === 'yes') {
			...
			res.redirect('/application/v0/org-error-fundedby')
		}
	...
	router.post('/application/v0/org-convictions-company-a', function (req, res) {
		...
		if (org_convict_company === 'no') {
			...
			res.redirect('/application/v0/org-duediligence-creditordebt')
		} else if (org_convict_company === 'yes') {
			req.session.data['tl_org_convictions'] = 'inprogress'
			res.redirect('/application/v0/org-duediligence-creditordebt')
		} else {
			res.redirect('/application/v0/org-error-convictions-company')
		}
	})


	// Org in debt with creditors?
	router.post('/application/v0/org-duediligence-creditordebt-a', function (req, res) {
		...
		res.redirect('/application/v0/org-duediligence-last3years')

	})

	// Apply to organisation 2 (safegurding and whistleblowing)
	router.post('/application/v0/org-duediligence-organisation-2-a', function (req, res) {
	...
	// Sign out
	router.get('/application/v0/signout-a', function (req, res) {
