// Routes for Organisation v0.1

module.exports = function (router) {

	// Sign in
	router.post('/organisation/v01/signin-a', function (req, res) {
		req.session.data['signedin'] = 'yes'
		res.redirect('/organisation/v01/task-list')
	})

	// Section complete - Select route
	router.post('/organisation/v01/select-route-a', function (req, res) {
		...
		if (selected_route === 'main') {
			...
			res.redirect('/organisation/v01/org-ico')
			...
			res.redirect('/organisation/v01/select-route-error')
		}
		
	})
  
	// ICO number?
	router.post('/organisation/v01/org-ico-a', function (req, res) {
		req.session.data['tl_org_details'] = 'inprogress'

		let org_ico = req.session.data['org-ico']

		if (org_ico === '12345678') {
			res.redirect('/organisation/v01/org-ukprn')
		} else {
			res.redirect('/organisation/v01/org-error-ico')
		}
	})

	// UKPRN?
	router.post('/organisation/v01/org-ukprn-a', function (req, res) {

		let org_ukprn = req.session.data['org-ukprn']

		if (org_ukprn === '11110000') { // Company - Happy Path
			res.redirect('/organisation/v01/org-confirm-companydetails')
		} else if (org_ukprn === '11110001') { // Company - Active with no website
			res.redirect('/organisation/v01/org-confirm-companydetails')
		} else if (org_ukprn === '11110002') { // Company - Inactive
			res.redirect('/organisation/v01/org-shutter-inactivecompany')
		} else if (org_ukprn === '11110003') { // Company - Also a charity
			res.redirect('/organisation/v01/org-confirm-companydetails')
		} else if (org_ukprn === '11110004') { // Not a company
			res.redirect('/organisation/v01/org-legalstatus')
		} else {
			res.redirect('/organisation/v01/org-error-ukprn')
		}
	})

	// Confirm company details
	router.post('/organisation/v01/org-confirm-companydetails-a', function (req, res) {

		res.redirect('/organisation/v01/org-trading')

	})

	// Started trading date
	router.post('/organisation/v01/org-trading-a', function (req, res) {
		...
		if (req.session.data['org-trading-day'] == "" || req.session.data['org-trading-month'] == "" || req.session.data['org-trading-year'] == "") {
			res.redirect('/organisation/v01/org-error-trading')
		} else {
			req.session.data['tl_org_details'] = 'completed'
			req.session.data['tl_org_people'] = 'next'
			res.redirect('/organisation/v01/org-confirm-pscs-company')
		}
	})
	
	// Section complete - Organisation Details
	router.post('/organisation/v01/org-confirm-pscs-a', function (req, res) {
		req.session.data['tl_org_people'] = 'completed'
		req.session.data['tl_org_convictions'] = 'next'
		res.redirect('/organisation/v01/org-convictions-company')
	})


	// Criminal Convictions - Company
	router.post('/organisation/v01/org-convictions-company-a', function (req, res) {
		...
		if (org_convict_company === 'no') {
			...
			res.redirect('/organisation/v01/org-duediligence-creditordebt')
		} else if (org_convict_company === 'yes') {
			req.session.data['tl_org_convictions'] = 'inprogress'
			res.redirect('/organisation/v01/org-duediligence-creditordebt')
		} else {
			res.redirect('/organisation/v01/org-error-convictions-company')
		}
	})


	// Org in debt with creditors?
	router.post('/organisation/v01/org-duediligence-creditordebt-a', function (req, res) {
		...
		res.redirect('/organisation/v01/org-duediligence-last3years')

	})

	// Apply to organisation 2 (safegurding and whistleblowing)
	router.post('/organisation/v01/org-duediligence-organisation-2-a', function (req, res) {

		req.session.data['tl_org_compliance'] = 'completed'
		req.session.data['tl_org_classification'] = 'next'

		res.redirect('/organisation/v01/org-classification')

	})

	// Organisation type
	router.post('/organisation/v01/org-classification-a', function (req, res) {
		...
		if (org_orgtype === 'education') {
			...
			res.redirect('/organisation/v01/org-classification-education')
			...
			res.redirect('/organisation/v01/org-classification-psb')
		} else if (org_orgtype === 'training') {
			req.session.data['tl_org_classification'] = 'inprogress'
			req.session.data['fha-exempt'] = 'no'
			res.redirect('/organisation/v01/org-classification-training')
		} else {
			res.redirect('/organisation/v01/org-error-classification')
		}
	})


	// Organisation type = Education
	router.post('/organisation/v01/org-classification-education-a', function (req, res) {
		...
		res.redirect('/organisation/v01/org-fundedby')

	})

	// Organisation type = Public Sector Body
	router.post('/organisation/v01/org-classification-psb-a', function (req, res) {
	...
	router.post('/organisation/v01/org-classification-training-a', function (req, res) {
	...
	// Funded by
	router.post('/organisation/v01/org-fundedby-a', function (req, res) {
		...
		if (org_fundedby === 'yes') {
			...
			res.redirect('/organisation/v01/org-error-fundedby')
		}
	...
	// Sign out
	router.get('/organisation/v01/signout-a', function (req, res) {

		req.session.data['signedin'] = 'no'
		res.redirect('/organisation/v01/signout')
